...
activation_threshold = np.max(mean_firing_rate, axis=1)*0.5#+np.std(mean_firing_rate, axis=1)*3#+0.5#+ 1 * np.std(mean_firing_rate, axis=1)
...
plt.xticks([0, x_fit[-1]],
            ['0', str(max(duration_list))], fontsize=10)
    ...
    if state_start_stop[i][1] < state_start_stop[j][1]:
        if abs(state_start_stop[i][2] - state_start_stop[j][1]) < dur:
            if int(state_start_stop[i][0]) != int(state_start_stop[j][0]):
                ...
                transition_estimated[int(state_start_stop[i][0]), int(state_start_stop[j][0])] += 1
                state_pare_list.append([int(state_start_stop[i][0]), int(state_start_stop[j][0])])
...
pl.plot(np.array(transition_prob).reshape(n_pat**2, 1), normed_transition_estimated.reshape(n_pat**2, 1), lw=0, marker='o',markersize = 4,clip_on=False,c='k')
