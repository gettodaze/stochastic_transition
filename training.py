...
for sim in range(n_sim):
    ...
    for i in range(n_pat):
        input_mat[int(i*N_E/n_pat):int((i+1)*N_E/n_pat), i] = strength_input

    """
    Training
    """
    PSP_amp = (tau_m-tau_syn)/((tau_m/tau_syn)**(-(tau_syn)/(tau_m-tau_syn))-(tau_m/tau_syn)**(-(tau_m)/(tau_m-tau_syn)))

    pat_start = 0 # time of next stimulus presentation
    pattern_id = 0 # initial state
    max_trace = np.zeros(N) + 1 # exponential trace for tracking maximum value of membrane potential
    tau_max = 10*1000 # time scale of max_trace

    delta_weight_save = 2000 # synaptic weights are saved every 2 sec.
    W_E_list = np.zeros((N_E,N_E,int(simtime_len/delta_weight_save)))  # array of saved W_E
    W_I_list = np.zeros((N_E, N_I, int(simtime_len / delta_weight_save))) # array of saved W_I

    W_E_between = np.ones((N,N))
    W_E_between[:,N_E:] *= 0
    for i in range(n_pat):
        W_E_between[int(i*N_E/n_pat):int((i+1)*N_E/n_pat),int(i*N_E/n_pat):int((i+1)*N_E/n_pat)]*=0
    W_E_within = np.zeros((N,N))
    W_E_within[:,N_E:] *= 0
    for i in range(n_pat):
        W_E_within[int(i*N_E/n_pat):int((i+1)*N_E/n_pat),int(i*N_E/n_pat):int((i+1)*N_E/n_pat)]=1
    ...
    for i in tqdm(range(simtime_len), desc="[training]"):
        ...
        y = x / max_trace # normalized membrane potential with max_trace. This will be replaced with homeostatis of weights?

        f = g(2*y) # firing rate of neurons. y is scaled with a constant value of 2.

        if i>0:
            error_filtered_E = (1.0 - 1 / 30000) * error_filtered_E + ((f[int(0*N_E/n_pat):int((2+1)*N_E/n_pat)] - g(E_term)[int(0*N_E/n_pat):int((2+1)*N_E/n_pat)])) / 30000
            error_filtered_I = (1.0 - 1 / 30000) * error_filtered_I + ((g(E_term)[int(0 * N_E / n_pat):int((2 + 1) * N_E / n_pat)] - g(I_term)[int(0 * N_E / n_pat):int((2 + 1) * N_E / n_pat)])) / 30000
        else:
            error_filtered_E = (f[int(0*N_E/n_pat):int((2+1)*N_E/n_pat)] - g(E_term)[int(0*N_E/n_pat):int((2+1)*N_E/n_pat)])
            error_filtered_I =(g(E_term)[int(0 * N_E / n_pat):int((2 + 1) * N_E / n_pat)] - g(I_term)[int(0 * N_E / n_pat):int((2 + 1) * N_E / n_pat)])
        ...
        W_E[0:N_E,:] = learning(W_E[0:N_E,:], g(E_term)[0:N_E], PSP, eps, f[0:N_E], W_E_mask[0:N_E,:])
        W_I[0:N_E,:] = learning(W_I[0:N_E,:], g(I_term)[0:N_E], PSP, eps, g(E_term)[0:N_E], W_I_mask[0:N_E,:])
    ...
    for kk in range(int(simtime_len/delta_weight_save)):
        if kk>0:
            ...
            LC_I.append(LA.norm(W_I_list[:, :, kk] - W_I_list[:, :, kk - 1])/(N_E*N_I))
...
np.savetxt('error_filtered_list_E_mean.txt', error_filtered_list_E_mean, delimiter=',')
np.savetxt('error_filtered_list_E_std.txt', error_filtered_list_E_std, delimiter=',')
np.savetxt('error_filtered_list_I_mean.txt', error_filtered_list_I_mean, delimiter=',')
np.savetxt('error_filtered_list_I_std.txt', error_filtered_list_I_std, delimiter=',')
